...
if __name__ == "__main__":
    ...
    cuvinte_spaii = [" ana ", " are ", " mere "]
    # Solutia:

    # print(list(map(lambda x: x.join(""), fara_spatii))
    ...
    numere = [5, 2, 8, 3, 9, 1]
    minim = (reduce(lambda x,y: x if x < y else y, numere))
    maxim = (reduce(lambda x,y: x if x > y else y, numere ))
    print(minim)
    print(maxim)
    print(maxim - minim)
    ...
    print(reduce(lambda x, y: x + len(y), cuvinte, 0))
